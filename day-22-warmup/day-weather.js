...
function printWeather(w) {
  ...
  for (var i = 0; i < w.hourlyForecast.length; i++) {
    if (w.hourlyForecast[i].hour === 0) {
      ...
      hour = w.hourlyForecast[i].hour - 12 + ':00 PM';
    }
    else {
      hour = w.hourlyForecast[i].hour + ':00 AM';
    }
    console.log(chalk.bgMagenta(hour));
    chance = '   Chance of rain: ' + w.hourlyForecast[i].chanceOfRain * 100 + '%';
    console.log(chance);
    temp = '   Temp: ' + w.hourlyForecast[i].temp;
    console.log(temp);
  }
}
